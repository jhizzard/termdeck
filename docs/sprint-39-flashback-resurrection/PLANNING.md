...
| **T3 — Project-tag write-path verification + chopin-nashville backfill** | Audit the cross-table project-tag flow. Map every write path that lands a memory in either `memory_items` (queried by Flashback) or the legacy `mnestra_*` tables. AND: ship the backfill migration that re-tags historical chopin-nashville pollution. The chopin-nashville tag is currently **96% polluted** (1,090 of 1,139 rows are mis-tagged termdeck/mnestra/rumen/podium/PVB content; only 49 legitimately chopin-nashville). Sprint 35/36 fixed the hook forward-only without backfilling; T3 closes that loop. NEW migration `011_project_tag_backfill.sql` (lane writes the SQL with `RAISE NOTICE` count probes; orchestrator applies at sprint close after reviewing). | `packages/server/src/rag.js`, `packages/server/src/mnestra-bridge/index.js`, `packages/server/src/session.js`, `~/Documents/Graciella/engram/src/recall.ts`, NEW `packages/server/src/setup/mnestra-migrations/011_project_tag_backfill.sql` |
