...
- **Claude Code installed and logged in.** TermDeck launches Claude Code sessions inside its panels by default. As of `termdeck@0.14.0`, **Codex / Gemini / Grok are also first-class lane agents** — install whichever CLIs you want available (`codex` from `openai/codex-plugin-cc`; `gemini` from `google-gemini/gemini-cli`; `grok` via `npm i -g grok-dev`). Each agent's instructional file convention is auto-honored: Claude reads `CLAUDE.md`, Codex + Grok read `AGENTS.md`, Gemini reads `GEMINI.md` (the latter two auto-generated from `CLAUDE.md` via `npm run sync:agents`). **Linux x64 install:** `npm install -g @anthropic-ai/claude-code --include=optional` — the platform binary is shipped as an `optionalDependencies` entry, so an environment with `omit=optional` configured globally (check `npm config get omit`) will install a `claude` stub that fails `claude --version`. The `--include=optional` flag is a no-op where it isn't needed; safe to always include on Linux x64. Same advice applies to `@jhizzard/termdeck-stack`. See `docs/GETTING-STARTED.md` § Linux x64 install hint for the full explanation.
