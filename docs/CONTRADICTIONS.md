...
| 9 | ~~Mnestra Flashback toast/panel shows parent directory path (`ChopinNashville`) instead of the project name from config.yaml — mnestra-bridge likely passes `cwd` path instead of the resolved project name~~ **Resolved (Sprint 21 T2):** code side fixed in Sprint 16 via `resolveProjectName` in mnestra-bridge; data side cleaned in Sprint 21 T2 — 75 `chopin-nashville` → `termdeck`, 29 `chopin-scheduler` → `scheduling`, 3 `gorgias` → `claimguard` in `rumen_insights`, plus 145 sub-0.10 confidence rows removed. | ~~`packages/server/src/mnestra-bridge/index.js` query context vs `~/.termdeck/config.yaml` project names~~ | **Resolved (Sprint 21)** |
