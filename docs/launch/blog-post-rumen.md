...
Right now (rumen@0.4.2) Rumen only generates insights in response to observed memories. A v0.5 path will add **question generation**: Rumen will occasionally write a memory of `source_type: question` — an open question it noticed across the store that it can't answer on its own. Those questions show up in TermDeck's Flashback surface so the next time you're in a related context, you see the question and can answer it (or decide it's not worth answering). The loop becomes: write → relate → synthesize → question → surface → answer → write. Not shipped yet; roadmap item.
