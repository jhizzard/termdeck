# Twitter/X launch thread — TermDeck v0.3.2 (Sprint 6 T4)
...
What it's not: Flashback only fires when the PTY output analyzer pattern-matches a known error class. No memory, no flashback. Mnestra currently needs Supabase + OpenAI; a local SQLite path is on the roadmap, not shipped in v0.3.2. 7/
...
v0.3.2 added startup health checks — Mnestra reachable, Rumen cron active, embedding provider live. A "Stack: OK" badge in the top bar so you know the full loop is wired before you start working. Also: automatic session transcript backup. 7b/
...
Solo dev, MIT, v0.3.2. Feedback very welcome — especially on the async loop design.
