# LinkedIn launch post — TermDeck v0.3.1 (Sprint 6 T4)
...
Honest limits, because I care more about this than the pitch: Flashback only fires when the PTY output analyzer pattern-matches a known error class. If it misses your error, no flashback fires. It's a shortest-path to a memory you already wrote — if the memory isn't there, the feature does nothing. Mnestra currently needs Supabase and OpenAI; a fully-local SQLite + local-embedding path is on the roadmap and not shipped in v0.3.1. Validated against one developer's store. No multi-user data yet.

v0.3.1 is live on npm as `@jhizzard/termdeck`. Try it with `npx @jhizzard/termdeck` — Node 18+, no global install needed. v0.3.1 added startup health checks (a "Stack: OK" badge in the top bar) and automatic session transcript backup. I'd love feedback from anyone who has hit the same "my tools don't remember" wall.
