...
## P0 — Outside-user blockers (urgent, schedule alongside 37/38)

**Brad is the only outside user of TermDeck besides Joshua.** Anything that prevents Brad from getting working ingestion is treated as a release-quality issue, not a backlog item.
...
- **🚨 Brad's empty-Mnestra ingestion fix — Mnestra-direct session-end hook.** Sprint 36 v0.8.0 shipped the MCP path migration + hook bundling, but the bundled `~/.claude/hooks/memory-session-end.js` delegates to `~/Documents/Graciella/rag-system/src/scripts/process-session.ts` — Joshua's private repo. Fresh users (i.e., Brad) get a hook that runs but silent-fails because `rag-system` doesn't exist on their box. Net effect: Brad's `pgvector` stays at 0 memories despite MCP wired and webhook bridge alive. **Joshua sent Brad a Mnestra-direct workaround hook on 2026-04-27 (`/tmp/brad-mnestra-hook.js`, ~120 LOC) — that proves the shape; the fix is to bring it home as the canonical bundled hook.** Two paths to close this in the published product:
  ...
  - **Scope:** small enough to NOT need a 4+1 sprint. Single-track fix. Bundle as Sprint 37.5 (between 37 and 38), OR fold into Sprint 38 as a fifth deliverable, OR ship as a v0.8.1 patch immediately after Sprint 37 closes. **Orchestrator's call at sprint planning time.**

---

## A. Correctness gaps (test debt + flagged bugs)

- **Analyzer-pattern fix sprint** — close the 4 pre-existing test failures: `stripAnsi removes standard CSI sequences (SGR colors)`, two `PATTERNS.error` session.js cases (regex too narrow on Node errno + HTTP 5xx), one fixture-dependent `project-bound flashback` test. None are Sprint-36 regressions; confirmed via stash + clean-main rerun. **Recommended as Sprint 39 if no urgency elsewhere.**
