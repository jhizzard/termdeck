...
**ADDITIONAL SUB-TASK (gate-blocker for v1.0.0, runs in parallel with T1-T4): Env-var propagation to PTY spawns.** Discovered at Sprint 47 close-out 2026-05-01 17:45 ET: 0 `session_summary` rows have ever landed in Mnestra because `memory-session-end.js` hits `env-var-missing` on `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` every time it fires. TermDeck server spawns Claude Code panels via `spawnTerminalSession` (`packages/server/src/index.js` ~767-820) but doesn't merge `~/.termdeck/secrets.env` into the PTY env. Two-layer fix: (a) TermDeck server explicitly reads + merges `secrets.env` into PTY env at spawn time so panels inherit the vars; (b) add a `secretsEnvSourceLine` snippet to the stack-installer's wizard that offers to add `set -a; source ~/.termdeck/secrets.env; set +a` to `~/.zshrc` so standalone Claude Code launches also work. This is sub-task scope ~50 LOC + 1-2 tests, can land in Sprint 48 alongside the lanes. **Without this fix, v1.0.0 ships with the file-based Stop hook still broken** — MCP `memory_remember` works fine but session-end auto-summaries never land.
