...
14. **Every Edge Function shipped by `init --rumen` has a corresponding pin-drift probe in `audit-upgrade.js`.** `npm publish` doesn't redeploy Supabase Edge Functions — the function source lives on Supabase's infrastructure and only re-deploys when the wizard runs `supabase functions deploy`. Any user who upgrades `@jhizzard/termdeck` without re-running `init --rumen` keeps a stale function pinned to whatever version was current at last deploy. The audit-upgrade probe set must include `probeKind: 'edgeFunctionPin'` for every deployed Edge Function (rumen-tick, graph-inference, plus any future additions), comparing the deployed `npm:<pkg>@<version>` import against the EXPECTED version (npm-registry resolver for `__RUMEN_VERSION__`-substituted imports; bundled-source resolver for static pins). Drift routes to `skipped[]` (YELLOW, non-blocking) with the explicit `init --rumen --yes` recommendation. (Class **O**.)
...
### #21 — Three v1.0.7 dogfood bugs (probe path off-by-one + macOS Docker bundler + stale repo link state) (2026-05-04 — orchestrator dogfood catch, closed in Sprint 52 v1.0.8 fold-in)

- **Reported:** 2026-05-04 ~16:30 ET by orchestrator during Sprint 52's post-publish dogfood on petvetbid. v1.0.7 had just shipped; running `npm install -g @jhizzard/termdeck@latest && termdeck init --rumen --yes` against petvetbid failed at the deploy step with `entrypoint path does not exist (supabase/functions/graph-inference/index.ts)` despite staging confirming the file was written. Three distinct bugs surfaced in series. All three closed in v1.0.8 within ~15 min wall-clock.
- **Symptom:** `init --rumen --yes` against any macOS+Docker+supabase-CLI-2.75.0-era install hits a deploy failure that the wizard's existing test suite couldn't catch. Specifically:
  1. Audit-upgrade reports the new `edgeFunctionPin` probe for graph-inference as YELLOW with `bundled source read failed at <bogus-path>: ENOENT` instead of the actual drift comparison — the probe's bundled-source resolver looked at `/usr/local/lib/node_modules/@jhizzard/packages/...` (no `termdeck/` segment).
  2. `supabase functions deploy graph-inference --project-ref <ref> --no-verify-jwt` fails with `entrypoint path does not exist` even though the file IS at `<stage>/supabase/functions/graph-inference/index.ts`.
  3. Joshua's TermDeck repo root had a 2026-04-15 stray `supabase/functions/rumen-tick/{index.ts,tsconfig.json}` left over from a manual debug run, which the supabase CLI silently preferred as cwd-relative state and contributed to per-function asymmetric deploy failures.
- **Root causes:**
  1. **Off-by-one path resolution:** `audit-upgrade.js`'s `probeEdgeFunctionPin` used `path.resolve(__dirname, '..', '..', '..', '..', '..')` (5 levels) when `__dirname` is `<root>/packages/server/src/setup/`. Five levels up lands at the npm scope dir (`@jhizzard/`); four lands at the package root. Tests passed because they injected `readFileImpl` and never exercised the production `fs.readFileSync(absPath)` path.
  2. **Docker bundler vs macOS /var/folders:** The Supabase CLI's default deploy path uses Docker to bundle the function. On macOS, Docker Desktop's default file-sharing allowlist does NOT include `/var/folders/` (the macOS user-temp dir that `os.tmpdir()` returns). The wizard stages to `os.tmpdir()`, the Docker bundler can't bind-mount the staged dir, and the "entrypoint path does not exist" error is the bundler's misleading way of saying "Docker can't see this path." Compounded by Joshua's local CLI being at 2.75.0 (current 2.98.1 — Joshua is 23 versions behind).
  3. **Repo-cwd contamination:** A 3-week-old debug session had run `supabase functions deploy` from the repo root and the CLI auto-created `supabase/functions/rumen-tick/` there. The wizard's `cwd: stage` invocation should have been cwd-isolated, but in some CLI version+platform combinations the CLI walks up looking for a `supabase/` parent and finds the stale repo-root state instead.
- **Why this hid through the v1.0.7 test suite:** The probe-path bug was masked by `readFileImpl` injection in the tests — the production code path `fs.readFileSync(<resolved-path>)` was never exercised against the actual globally-installed file system layout. The Docker bug was masked by every test stubbing the `supabase` CLI subprocess (no test runs the real binary). The stale-repo-state bug was masked by tests starting from a clean fixture, never from "an existing developer machine with old debug residue." **Three categories of test-coverage absence: (a) production-vs-mocked filesystem layout, (b) external-CLI subprocess actually executing, (c) starting-state realism.** Worth canonizing as a coverage-pattern in the pen-test sprint.
- **Fix (v1.0.8 fold-in, ships 2026-05-04):**
  1. `audit-upgrade.js`: 5 `..` → 4 `..`. Comment block explains the npm-scope-dir gotcha.
  2. `init-rumen.js:417`: every `supabase functions deploy` invocation gains `--use-api`. The Management API path bypasses Docker entirely and works regardless of CLI version, Docker file-sharing config, or `os.tmpdir()` layout. The user-visible "Running:" banner reflects the flag.
  3. Stale repo `supabase/` removed locally. No code fix needed (the wizard's staging is correctly cwd-isolated; the issue was environmental).
  4. NEW `tests/init-rumen-project-ref.test.js` `--use-api` regression pins (2 new tests): asserts the deploy argv contains `'--use-api'` AND the user-visible step banner reflects it.
- **Class:** **O — Deployed-state drift, expanded scope.** All three bugs are sub-cases of the same root pattern Sprint 52 named: the gap between what's published and what actually runs in deployed state. The probe-path bug means our DETECTION of drift was itself drifting. The Docker bundler bug means our REMEDIATION mechanism didn't work on every platform. The stale-cwd bug means PRIOR deployed state contaminated NEW deploys. Together they reinforce: every Class O probe needs (a) a production-FS exercising test, (b) a real-CLI subprocess test (or platform matrix coverage), (c) a "run from a contaminated starting state" test. Pre-ship checklist item #14 expanded: not just "every Edge Function has a pin probe" but also "every probe is exercised against the real production FS layout" + "every external CLI subprocess is exercised on the target platform."

### #20 — Deployed-state pin drift between npm-published packages and Supabase Edge Functions (2026-05-04 — Codex T4-CODEX live catch during Sprint 51.5b, closed in Sprint 52)

- **Reported:** 2026-05-04 15:22 ET by Codex (T4-CODEX auditor) during Sprint 51.5b dogfood Phase G. Independent probe via `supabase functions download rumen-tick --project-ref luvvbrpaopnblvxdxwzb --use-api -o /tmp/sprint-52-deployed-rumen-tick` followed by `grep -E "npm:@jhizzard/rumen@" /tmp/sprint-52-deployed-rumen-tick/index.ts` returned `npm:@jhizzard/rumen@0.4.0`. Concurrent `npm view @jhizzard/rumen version` returned `0.4.5`. Triggered Sprint 52 (single-lane direct, ~1h wall-clock). Closed in v1.0.7.
- **Symptom:** A user who upgrades `@jhizzard/termdeck` via `npm install -g @jhizzard/termdeck@latest` and runs `termdeck init --mnestra` to refresh their hook + run audit-upgrade probes sees every SQL probe pass GREEN — yet their deployed `rumen-tick` Edge Function is still pinned to whatever rumen version was current at their LAST `init --rumen` run, multi-patch-stale. There's no surface in the wizard that tells them to also re-run `init --rumen` to refresh the deployed function source. Insights flow may continue (if the deployed version is functionally adequate) or silently degrade. Joshua's petvetbid: deployed `0.4.0`, current `0.4.5` (3 patches behind). Brad's jizzard-brain almost certainly has the same drift.
- **Root cause:** `npm publish` updates the npm registry; it does **not** touch Supabase. The Edge Function source on Supabase only re-deploys when the wizard explicitly runs `supabase functions deploy <slug>`. The wizard's existing audit-upgrade probe set (`packages/server/src/setup/audit-upgrade.js` PROBES) covered SQL artifacts (mig presence, RPC presence, column presence, cron jobname presence) plus Sprint 51.6 T3's `functionSource` probes (deployed-source feature-marker presence — for SUPABASE_DB_URL fallback) — but **no probe compared the deployed `npm:<pkg>@<version>` import line against the current published version**. Feature-marker probes catch "missing capability X"; they don't catch "old version still works but is N patches stale."
- **Why this hid through Sprints 51.5/51.6/51.7/51.8/52.1/51.9:** every previous audit-upgrade addition was schema-side or feature-marker-side. The deployed-state-vs-npm-state mental model wasn't part of any sprint's design space until Codex's MCP-wired adversarial probes started downloading deployed function bodies independently. The 233/321 `rumen_insights` flatline since 2026-05-01 misled the team into focusing on schema-side bugs (the v1.0.x onion's six prior incidents) — the pin drift was orthogonal infrastructure-deployed-state drift that the schema-only probe set was structurally blind to. **Pattern parallel to ledger #15 (architectural omission): an entire dimension of state was untracked.**
- **Fix (Sprint 52, ships v1.0.7 wave 2026-05-04):**
  1. NEW `probeKind: 'edgeFunctionPin'` in `packages/server/src/setup/audit-upgrade.js`. Two PROBES entries (rumen-tick + graph-inference). Probe set grows from 10 → 12.
  2. Probe mechanism: download deployed body via Supabase Management API (`GET /v1/projects/<ref>/functions/<slug>/body` — same endpoint as Sprint 51.6 T3's `functionSource` probe), grep `npm:<pkg>@<version>` from import line, compare against EXPECTED version. Two resolver shapes: `npmRegistry` (run `npm view <pkg> version` — used for rumen-tick because bundled has `__RUMEN_VERSION__` placeholder substituted at deploy) or `bundledSource` (read bundled file, grep same shape — used for graph-inference because postgres pin is static).
  3. On drift: routes to `skipped[]` (YELLOW band, non-blocking) with recommendation containing both versions and `termdeck init --rumen --yes`. Fail-soft on every error path (Management API HTTP error, network throw, missing access token, npm view failure, unparseable body, bundled file unreadable) — all degrade to `skipped[]` with the underlying reason.
  4. Probe runs in BOTH `init --mnestra` and `init --rumen` audit-upgrade phases — users running `init --mnestra` to refresh hooks also get told about Edge Function staleness without needing to remember `init --rumen`. The `init --rumen` deploy step itself is already unconditional (since v0.6.x — `packages/cli/src/init-rumen.js:1024` calls `deployFunctions(...)` every wizard pass with no first-install gate); v1.0.7 just makes the staleness visible.
  5. NEW test file `tests/audit-upgrade-edge-function-pin.test.js` (14 tests). Coverage: in-sync (both resolvers), drift (both resolvers, recommendation must include both versions + `init --rumen`), 404 / network throw / missing token / npm view failure / parse failures / bundled unreadable → skipped fail-soft, integration via `auditUpgrade()` confirming pin probes route to `skipped[]` not `missing[]` and never auto-apply.
  6. Pre-ship checklist gains item #14: every Edge Function shipped by `init --rumen` must have a corresponding `edgeFunctionPin` probe in `audit-upgrade.js` PROBES.
- **Out of scope — deferred to Sprint 53:** the 233/321 `rumen_insights` flatline is **not** closed by v1.0.7. Sprint 52 detects pin drift; a redeploy from `0.4.0` → `0.4.5` does not, by itself, restart insights flow. The deeper bug is the Rumen picker shape mismatch on `memory_items` 1-row-per-session: the picker at `~/Documents/Graciella/rumen/src/extract.ts:55-77` assumes the pre-Sprint-51.6 multi-row-per-session pattern; the new bundled hook writes one row per session. The fix is a pivot to read from `memory_sessions` directly (now authoritative post-mig-017) — a Rumen-package change in a separate publish wave, queued as Sprint 53.
- **Class:** **O — Deployed-state drift between npm-published packages and Supabase-deployed Edge Functions** (NEW). Pattern: an `npm publish` updates the registry but does NOT update separately-hosted execution surfaces (Supabase Edge Functions, Vercel functions, AWS Lambda, any externally-deployed runtime that pulls from npm at deploy time rather than at install time). Generalizes to any project that publishes a runtime artifact AND has a deploy-to-managed-infra step that's user-triggered rather than CI-triggered. Pre-ship checklist item #14 makes this an explicit invariant: every Edge Function deploy path needs a corresponding pin-drift probe. Distinct from Class H (migration-runner blindness — that's database-side schema drift) and Class M (architectural omission — that's a write-path absence). Class O is **runtime-deployed-state drift between an npm-published artifact and a separately-hosted execution surface**.

### #19 — Mig 016 `cron.*` schema dependency on fresh Supabase projects (2026-05-04 — T3 live catch during Sprint 51.5b, folded into Sprint 51.9)

- **Reported:** 2026-05-04 14:55 ET by T3 during Sprint 51.5b dogfood Phase B (canonical fresh-install path). Folded into Sprint 51.9 (v1.0.6) wave alongside ledger #18 within ~50 min.
- **Symptom:** `termdeck init --mnestra` against a freshly-provisioned Supabase project (clean tmp HOME, no prior install) hits exit 5 on `016_mnestra_doctor_probes.sql` with `relation "cron.job_run_details" does not exist`. Migrations 001-015 apply cleanly first; mig 017 never reached.
- **Root cause:** Mig 016 (Sprint 51.5 T2 — `mnestra doctor` SECURITY DEFINER probe wrappers) creates two `language sql` functions referencing `cron.job_run_details` and `cron.job` directly. Postgres parse-time-resolves identifiers in `language sql` function bodies, so the CREATE itself fails when the `cron` schema doesn't exist. Fresh Supabase projects do NOT have `pg_cron` enabled by default — the default extension set is `pg_stat_statements, pg_trgm, pgcrypto, plpgsql, supabase_vault, uuid-ossp, vector`. Only Rumen's mig 002 issues `CREATE EXTENSION pg_cron`. **Specifically impacts the canonical "Tier 2 memory layer (standalone Mnestra)" path** that the wizard's own banner promises (`init-mnestra.js:175-187`): users running `init --mnestra` BEFORE `init --rumen` (or skipping rumen entirely) hit a hard exit-5.
- **Why this hid through Sprints 51.5/51.6/51.7/51.8/52.1:** every existing developer machine + Brad's jizzard-brain has rumen installed. Joshua's daily-driver petvetbid + Brad's jizzard-brain BOTH had pg_cron from prior `init --rumen` runs, so mig 016 always parse-resolved cleanly. The bug only surfaces on a fresh Supabase project that NEVER had Rumen installed — a state no one had reproduced until T3's Sprint 51.5b Phase B dogfood. **The "fresh user, fresh project" probe wasn't part of any sprint's acceptance criteria until 51.5b.**
- **Fix (Sprint 51.9 fold-in, ships v1.0.6 wave 2026-05-04):**
  1. Mig 016 split into cron-INDEPENDENT half (column_exists, rpc_exists, vault_secret_exists — emit unconditionally) + cron-DEPENDENT half (cron_runs, cron_job_exists — wrapped in `do $cron_guard$ ... if exists pg_extension where extname='pg_cron' then execute ... end if; end $cron_guard$;`). The do-block uses nested dollar-quoting (`$cron_guard$/$stmt$/$body$`) so the function bodies inside EXECUTE statements parse cleanly.
  2. Doctor's cron probes return the existing `unknown` band when the cron wrappers don't exist (Sprint 51.5 T2 contract holds — graceful degradation).
  3. Mirrored byte-identical to `~/Documents/Graciella/engram/migrations/016_mnestra_doctor_probes.sql`.
  4. NEW shape regression suite at `tests/migration-016-shape.test.js` (7 tests). Pins: file existence, byte-identical to Mnestra-repo primary, do$cron_guard$ presence checking pg_extension, cron-dependent functions ONLY inside guard, cron-independent OUTSIDE guard, dependent grants inside guard, no `CREATE EXTENSION pg_cron` (rejected Option B).
  5. T3's lane brief (line 586) recommended this exact fix shape over two alternatives: Option B (auto-enable pg_cron from migration — couples mnestra to a cron extension it doesn't otherwise need + only postgres role can CREATE EXTENSION on Supabase) and Option C (move mig 016 to Rumen-side — breaks doctor's standalone-mnestra promise).
- **Class:** **A — Schema drift** (seventh incident, second within a single day). Sister of #17 + #18. Pattern: a migration assumes a schema (`cron`, `vault`, etc.) is available without checking the prerequisite extension. Same shape as the v0.6.x-era `match_memories` + `memory_hybrid_search` drift cousins — function-body identifier resolution at CREATE time fails on absent prerequisites. **Pre-ship checklist update needed (Sprint 52+ candidate):** every migration that references a non-default schema (`cron.*`, `vault.*`, anything beyond `public`/`information_schema`) must wrap CREATE statements in `do$$ if exists pg_extension where extname='<ext>' then ... end if; end $$;` OR emit `CREATE EXTENSION IF NOT EXISTS <ext>;` first. Linter candidate.
...
| **O** | Deployed-state drift (npm-published artifact ≠ separately-hosted execution surface) | Does `npm publish` update every execution surface the user runs? If not, what user-triggered redeploy step bridges the gap, and is its omission visible in the audit-upgrade probe set? | `npm install -g @latest` updates the local bin but leaves Supabase Edge Functions / Vercel functions / AWS Lambda pinned to the prior deploy. User runs `init --mnestra` to refresh hooks, sees all SQL probes pass GREEN, never knows their Edge Function is N patches stale. Insights silently degrade or stop. |
