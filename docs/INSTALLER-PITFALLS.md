...
- **Sprint 51.6.1 / v1.0.3 follow-up (small):** the wizard's auto-invocation of `refreshBundledHookIfNewer()` doesn't actually fire — call site at `init-mnestra.js:677` is structurally correct, function works in isolation (Codex confirmed via `node -e "...refreshBundledHookIfNewer({dryRun:true})"`), but Joshua's `termdeck init --mnestra` run did not actually invoke it. ~30 min hotfix tomorrow. Plus a metadata-completeness pass: the v1 bundled hook intentionally omits `started_at` / `duration_minutes` / `facts_extracted`-from-transcript-parsing — the rag-system writer populated those by parsing transcript timestamps. Worth folding into the same v1.0.3 since we're already touching the bundled hook.
